...
for char in chars:
    sum = 0
    for item in pick_rates[char]:
        sum += item
    char_metric_dictionary[char]["pick_rate"] = sum / len(pick_rates[char])
...
for char in chars:
    ax2.scatter(char_metric_dictionary[char]["pick_rate"], char_metric_dictionary[char]["win_delta"], label=char_dict[char], marker = 5, s=75)
ax2.legend()
