...
def choice_available(state_desc):
    ...
    # Healers might have a second heal target who is ststrategy_generatorstrategy_generatorstrategy_generatorunned...
    if state_desc[6] > "0" and int(state_desc[6]) < p2_sum and state_desc[7] != "5":
        return True
    # one actor, one target, no special cases -> one action.
    return False

def run(file_prefix, trip, i):
    ...
    total_actions = 0
    voided_actions = 0
    file_to_write = file_prefix + "/" + trip + "_strategy_" + str(i) + ".txt"
    f = open(file_to_write, "w")
    f.write("// Generated strategy for " + trip + " at a later iteration\n")
