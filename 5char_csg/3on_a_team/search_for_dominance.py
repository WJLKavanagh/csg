...
def run_analysis(config, output):
    ...
    for trip in trips:
        ...
        file_name = trip + "_dominance_check.prism"
        ...
        f.write(open("../model_sections/3file_prefix.txt").read())
        for char in chars:
            if char in trip:
        ...
        f.write(open("../model_sections/choose_3chars.txt").read())
        f.write("\n\t[coin_flip] turn = 0 & p2K*p2A*p2W*p2R = 0 ->\n")
        ...
        "/tmp.sta -javamaxmem 8g -nopre -maxiters 300000 > " + output + "/log.txt")
        ...
        results[trip] = {"res":minimax, "opp":found_opp}
