...
def loop_check(j):
    ...
    for p in range(1, j-1):
        all_clear = False
        for trip in trips:
            if not filecmp.cmp(output+"/"+trip+"_strategy_"+str(j-1)+".txt", output+"/"+trip+"_strategy_"+str(p)+".txt", shallow=False):
                all_clear = True
                break
        if not all_clear:
...
while loop_check(k) == 0 or (best_probability > 0.5 and loop_check(k) == 1):
    ...
    for trip in trips:
        ...
        os.system("prism output/" + trip + "_vs_" + str(k-1) + ".prism ../../properties/mdp.props \
        -prop 1 -exportadvmdp " + output + "/tmp.tra -exportstates " + output + \
